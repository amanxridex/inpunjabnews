...
categories = {
    ...
    'Breaking': str(uuid.uuid4())
}

articles = []
titles_seen = set()

for card in soup.select('.news-card') + soup.select('.side-card') + soup.select('.hero-content'):
    title_el = card.select_one('.news-card-title, .side-title, .hero-title')
    ...
    cat_el = card.select_one('.cat-pill, .side-cat, .hero-tag')
...
with open('seed.sql', 'w', encoding='utf-8') as f:
    f.write('TRUNCATE public.articles, public.categories CASCADE;\n')
